              ...
              <div class="row g-3">
                ...
                <template v-if="bookingMode === 'appointment'">
                  ...
                  <div class="col-md-6">
                    ...
                    <input v-model="bookingForm.customerName" type="text" class="form-control">
                  </div>
                  <div class="col-md-6">
                    <label class="form-label">{{ $t('admin.phone') }}</label>
                    <input v-model="bookingForm.customerPhone" type="tel" class="form-control">
                  </div>
                  <div class="col-12">
  ...
  data() {
    return {
      ...
      bookingForm: {
        ...
        endDate: (() => { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}` })(), // end date for block mode
      ...
      todayDate: (() => { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}` })(),
      availableSlots: [],
      primaryBarber: null,
...
/* Safari / iOS: enforce equal slot height */
@media (max-width: 768px) {
  @supports (-webkit-touch-callout: none) {
    .date-time-schedule .slots-column {
...
/* Ensure both slots are always equal size */
.half-hour-slot.slot-first,
.half-hour-slot.slot-second {
  flex: 1 1 50%;
  min-height: 25px;
...
@media (max-width: 576px) {
  ...
  /* CRITICAL: Ensure slots-column uses flexbox properly (Safari/iOS) */
  .slots-column {
    display: flex !important;
    -webkit-flex-direction: column !important;
    flex-direction: column !important;
...
@supports (-webkit-touch-callout: none) {
  ...
  /* iOS Safari half-hour slot optimizations */
  .half-hour-slot {
    -webkit-tap-highlight-color: rgba(16, 185, 129, 0.3);
    touch-action: manipulation;
    -webkit-touch-callout: none;
  ...
  @media (max-width: 768px) {
    ...
    /* Ensure slots are fully tappable */
    .half-hour-slot,
    .half-hour-slot .empty-slot,
    .half-hour-slot .appointment-block {
      position: relative;
      z-index: 1;
    }
  }
